...
syn keyword sqlKeyword	access add  asc begin by check cluster column
...
syn keyword sqlKeyword	nocompress nowait of offline online start
syn keyword sqlKeyword	successful synonym table then to trigger uid
syn keyword sqlKeyword	unique user validate values view whenever when
...
syn region  sqlParent   start="(" end=")"
syn keyword sqlOperator	not and or  return 
syn keyword sqlOperator	in any some all between exists
syn keyword sqlOperator	like escape
syn keyword sqlOperator union intersect minus
syn keyword sqlOperator prior distinct as on
syn keyword sqlOperator	sysdate out

syn keyword sqlException no_data_found too_many_rows others zero_divide
syn keyword sqlStatement alter analyze audit comment commit create replace 
...
syn keyword sqlFrom     join inner left outer right 
